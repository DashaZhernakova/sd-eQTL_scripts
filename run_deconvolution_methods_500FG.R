...
bulk_expr_tpm <- as.matrix(read.delim("data/1508_Li_RNAseq.expression.genelevel.v75.htseq.txt.table.gz.TPM.txt", row.names = 1, header = T, as.is = T, check.names = F, sep = "\t"))
...
write.table(decon_abis_res, file = "cell_counts/500FG.TPM.nnls.ABIS.txt", sep = "\t", quote = F, col.names = NA)
write.table(decon_lm22_res, file = "cell_counts/500FG.TPM.nnls.LM22.txt", sep = "\t", quote = F, col.names = NA)

write.table(decon_dtangle_abis_res, file = "cell_counts/500FG.TPM.dtangle.ABIS.txt", sep = "\t", quote = F, col.names = NA)
write.table(decon_dtangle_lm22_res, file = "cell_counts/500FG.TPM.dtangle.LM22.txt", sep = "\t", quote = F, col.names = NA)
